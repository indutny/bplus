...
int bp__page_insert(bp_tree_t* t, bp__page_t* page, const bp__kv_t* kv) {
  ...
  if (res.child == NULL) {
    ...
    if (ret == BP_ESPLITPAGE) {
      ...
      page->keys[res.index].csize = res.child->csize;
  ...
  if (page->length == t->head.page_size) {
    if (page == t->head_page) {
      /* split root */
      bp__page_t* new_root;
      bp__page_create(t, 0, 0, 0, 0, &new_root);
...
int bp__page_split(bp_tree_t* t,
  ...
  bp__page_create(t, child->is_leaf, 0, 0, 0, &left);
  bp__page_create(t, child->is_leaf, 0, 0, 0, &right);
  ...
  parent->keys[index].csize = left->csize;
